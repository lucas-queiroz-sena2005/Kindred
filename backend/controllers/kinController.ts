...
/**
 * Validates and sanitizes query parameters for the getKinList endpoint.
 * Enforces strict typing, whitelisting of parameters, and default values.
 *
 * @param query - The raw query object from the request.
 * @returns A typed object containing validated filter, sortBy, limit, and offset.
 * @throws ApiError if unknown parameters are present or values are invalid.
 */
...
/**
 * Retrieves a paginated list of users ("kins") sorted by profile similarity.
 *
 * @route GET /api/kin
 * @access Private
 */
...
/**
 * Compares the authenticated user's profile with a specific target user.
 * Returns similarity scores for the overall profile and individual feature segments.
 *
 * @route GET /api/kin/compare/:targetId
 * @access Private
 */
export async function compareKin(
  ...
  try {
    ...
    if (!/^\d+$/.test(targetIdParam)) {
      throw new ApiError("Target ID must be a valid positive integer.", 400);
    }

    const targetId = parseInt(targetIdParam, 10);

    if (targetId < 1) {
      throw new ApiError("Target ID must be a positive integer.", 400);
    }
