...
export default function SimpleTierRow({
  tier,
  onSelectMovie,
}: SimpleTierRowProps): React.ReactElement {
  ...
  return (
    <div className="flex items-stretch min-h-[72px] sm:min-h-[100px] mb-1 border border-gray-700 rounded min-w-0">
      <div
        className={`flex items-center justify-center w-16 sm:w-28 text-white font-bold text-sm sm:text-xl flex-shrink-0 ${tierColorClass}`}
      >
        {tier.title}
      </div>
